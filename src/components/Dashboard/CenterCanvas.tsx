...
import { SceneLights, GroundGlow, StarField } from '../ThreeD/SceneSetup';
...
function CameraController({ selectedProvince }: { selectedProvince: string | null }) {
  ...
  useFrame((_, delta) => {
    if (controlsRef.current) {
      controlsRef.current.target.lerp(targetPos.current, Math.min(delta * 3, 0.1));
  ...
  return (
    <div style={{ width: '100%', height: '100%', position: 'relative' }}>
      ...
      <div style={{
        ...
        color: '#B8B2A6', fontSize: 10, opacity: 0.5,
        pointerEvents: 'none',
      }}>
